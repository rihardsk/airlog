...
use nrf52840_hal::pwm as nrf_pwm;

/// The current Pwm traits in embedded_hal suck, but i don't want to switch
/// everything to the alpha version, which has the new traits, so i'll copy them
/// here and implement them (or some of them) myself for now
...
impl<'a, T> ErrorType for nrf_pwm::PwmChannel<'a, T>
where
    T: nrf_pwm::Instance,
{
...
impl<'a, T> SetDutyCycle for nrf_pwm::PwmChannel<'a, T>
where
    T: nrf_pwm::Instance,
{
    fn get_max_duty_cycle(&self) -> u16 {
